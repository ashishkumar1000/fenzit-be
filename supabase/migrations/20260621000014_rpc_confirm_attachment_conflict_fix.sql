-- Story 4.3 — Server-Side Conflict Resolution (FR-18) — code-review corrections.
--
-- Supersedes migration 000013. Forward-only restatement of confirm_attachment
-- that corrects the conflict_resolved audit semantics surfaced in code review:
--
--   F3/F2: The pure idempotent-replay path (v_upload.status = 'confirmed' — the
--          SAME upload_id re-delivered, e.g. a Worker webhook retry) replaces no
--          data. It is a duplicate delivery, not a last-write-wins conflict, so it
--          must NOT emit a conflict_resolved log. (000013 logged it here with a
--          self-referential replaced_upload_id = p_upload_id.) A conflict is, by
--          definition, a DISTINCT upload displacing an existing slot.
--
--   F4:    The signature concurrent-confirm race — two DIFFERENT uploads both miss
--          the existing-row check and both INSERT; the loser hits the partial
--          unique index (uniq_attachments_signature_per_job, migration 010) and
--          raises 23505. That loser IS a genuine last-write-wins conflict but
--          000013 produced no audit trail for it. Now logged.
--
--   F5:    The signature-UPDATE path's COALESCE(v_existing_id, p_upload_id) was
--          dead code: PL/pgSQL FOUND is true only when the UPDATE matched a row,
--          and that row was captured into v_existing_id by the immediately-prior
--          SELECT on the same predicate — so v_existing_id is never NULL when the
--          log fires. The fallback is removed; the log references v_existing_id
--          directly (the displaced upload).
--
-- Net effect: conflict_resolved fires exactly when a distinct upload displaces or
-- races for a slot (signature replace, or signature concurrent-INSERT loser),
-- never on same-upload re-delivery. Photos never "replace a slot" (each photo is
-- its own row); a re-delivered photo hits only the idempotent path → no log.
...
BEGIN
  ...
  --    is a duplicate delivery, NOT a conflict — no conflict_resolved log (F3/F2).
  ...
  IF v_upload.attachment_type = 'signature' THEN
    ...
    IF FOUND THEN
      -- AC4: an existing signature (a DISTINCT prior upload) was replaced →
      -- last-write-wins conflict. FOUND is true only when the UPDATE matched the
      -- row the SELECT above captured, so v_existing_id is non-NULL here (F5).
    ...
    ELSE
      ...
      -- winner's row already satisfies AC#7 — resolve idempotently AND record the
      -- conflict (F4: the loser is a genuine last-write-wins event) instead of
      -- letting 23505 bubble to a 500 / Worker poison-retry.
      BEGIN
