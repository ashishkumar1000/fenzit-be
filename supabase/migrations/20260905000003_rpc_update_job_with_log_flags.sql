-- Story 3.8: re-issue update_job_with_log so the completion flags are editable.
-- Append-only history: never edit an applied migration. The two new params are
-- appended at the END of the list (after p_priority) — Postgres RPC params are
-- positional, and inserting mid-list would silently reorder existing callers.
-- COALESCE: a NULL param leaves the flag unchanged (PATCH semantics; absent =
-- unchanged). PT409/PT422 guards and the reassignment log are copied verbatim
-- from 20260621000004.
...
BEGIN
  ...
  ELSE
    ...
    SET description          = COALESCE(p_description, description),
        ...
        require_completion_signature  = COALESCE(p_require_completion_signature, require_completion_signature),
        updated_at           = now()
    WHERE id = p_job_id;
