-- Story 7-1: Add job-level location-capture toggle
ALTER TABLE jobs ADD COLUMN capture_location_on_steps BOOLEAN NOT NULL DEFAULT true;

-- Story 7-1: Update create_job_with_log RPC to accept location capture toggle
...
CREATE FUNCTION create_job_with_log(
  ...
  p_actor_id                UUID,
  p_year                    INT,
  p_capture_location_on_steps BOOLEAN DEFAULT true
)
RETURNS SETOF jobs
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO jobs (
    ...
    description, notes_for_technician, capture_location_on_steps
  ) VALUES (
    ...
    p_description, p_notes_for_technician, p_capture_location_on_steps
  );
